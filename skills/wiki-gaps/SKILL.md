...
**Several vaults connected?** Each is a part of one brain (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-query/references/parts.md`). This skill changes nothing in any vault, so it may read across them — but a gap is only a gap if no part holds it: check the others by name before naming one, route by what each part's §1 is for, honour a named scope exactly, and say which parts you looked in. A gap another part already answers is not a gap; it is something to quote across, with its part and vault id. The offer in step 4 changes too: fetching a source is a capture into the part it belongs to, and ingesting it happens from that part's own project (`parts.md`, *A project that combines several wikis*) — say so in the offer.
