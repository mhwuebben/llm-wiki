...
2. **Check it against the schema's out-of-scope list** — schema §1, the owner's own list, and nothing else: what it doesn't name is not blocked, however sensitive it looks. An item that matches a line isn't filed on your judgement that it is probably fine, and isn't refused on it either: **ask once** — on a card where the session has one — naming the line it matches, with two choices: **File it anyway** or **Skip it**. Filed anyway, it goes ahead like any other item: add `scope: "override — <the line it matched>"` to its provenance block (quoted, because the line may contain colons), and ingest carries it onto the source page and propagates it as usual. **Said up front** — *"capture this anyway"*, *"force it"*, *"file it even if it's out of scope"* — skip the question and do the same. Several items that match: one card listing them, one answer for each. Unattended, nobody can answer: leave it where it came from and name it, with its line, in the report or digest.
