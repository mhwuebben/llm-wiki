...
- **what looks generated:** an API reference built from code, a database schema dump, a changelog, vendored third-party docs. Each is better left out, or captured as a single source — say which you see. **Documentation someone maintains is a source even when a tool writes it** — docs regenerated by an AI assistant or a docs generator from decisions people made: import it, and let ingest notice where a new version repeats what the wiki itself concluded (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-ingest-pending/references/re-captures.md`, *Echoes*)
- **what the schema's out-of-scope list names** (wiki-capture-only, step 2): files whose names or kinds match one of its lines — proposed for the exclude list, one line for them all, and kept in only if the owner says so; a shared drive often holds some
...
- **A line can point at an existing copy** instead of a new one, with a note after it: `(moved)` — the same content as a file no longer at its old path: the old path is the one whose last line with a copy names this same copy, and it gets its `gone` line in the same sync; `(same text)` — only whitespace changed; or no note, where the only news is a git date that has turned up since. None of them adds a file to `raw/` or anything to ingest.
...
last() { P=$1 awk '{ n = split($0, a, " · "); if (n < 3 || substr(a[1], 1, 2) != "- ") next
  ...
  if (p == ENVIRON["P"]) { f = a[n-1]; if (f != "gone") { k = f; g = a[n]; sub(/ → .*$/, "", g); c = a[n]; sub(/^.* → /, "", c) } } }
  END { print (f == "" ? "-" : f) " " (k == "" ? "-" : k) " " (g == "" ? "-" : g) " " (c == "" ? "-" : c) }' "$V/$R"; }
paths() { awk '/^- / { n = split($0, a, " · "); if (n < 3) next; p = substr(a[1], 3); for (i = 2; i <= n - 2; i++) p = p " · " a[i]; print p }' "$V/$R" | sort -u; }
nws() { tr -s ' \t\r\n' '    ' < "$1" | sed 's/^ //; s/ $//' | (sha256sum 2>/dev/null || shasum -a 256) | cut -c1-16; }   # its text, spacing ignored
: > "$TMP.ev"; : > "$TMP.gd"; : > "$TMP.miss"
if [ "$(git -c safe.directory='*' rev-parse --is-inside-work-tree 2>/dev/null)" = true ]; then
  { git -c safe.directory='*' -c core.quotePath=false diff --name-only --relative HEAD -- .   # uncommitted or untracked: no git date
    git -c safe.directory='*' -c core.quotePath=false ls-files --others --exclude-standard -- .; echo '@-'
...
[ -n "$FRESH" ] || paths | while IFS= read -r rel; do         # recorded files no longer at their path: candidates for a move
  [ -e "./$rel" ] && continue; st=$(last "$rel"); f=${st%% *}; st=${st#* }; k=${st%% *}
  [ "$f" = gone ] && f=$k; [ "$f" = - ] || printf '%s\t%s\n' "$f" "$rel" >> "$TMP.miss"
done
...
sort | while IFS= read -r rel; do
  ...
  if [ -z "$FRESH" ] && [ -n "$(find "./$rel" -mmin -2 -mmin +0 2>/dev/null)" ]; then echo "RECENT: $rel" >> "$TMP.ev"; continue; fi   # still being written: the next sync
  fp=$( (sha256sum 2>/dev/null || shasum -a 256) < "./$rel" | cut -c1-16)
  [ ${#fp} -eq 16 ] || { echo "NOT READ: $rel"; continue; }
  st=$(last "$rel"); f=${st%% *}; st=${st#* }; k=${st%% *}; st=${st#* }; lg=${st%% *}; kc=${st#* }
  gd=$(P=$rel awk -F'\t' '$1 == ENVIRON["P"] { print $2; exit }' "$TMP.gd"); gd=${gd:--}
  kf=${kc%% *}; att=$(printf '%s' "$kc" | sed -n 's/.*\( (attachment of [^)]*)\).*/\1/p')
  if [ "$fp" = "$f" ]; then                                     # unchanged — but a git date that has turned up since is recorded
    [ "$lg" = - ] && [ "$gd" != - ] && { printf -- '- %s · %s · %s → %s%s\n' "$rel" "$fp" "$gd" "$kf" "$att" >> "$V/$R"; echo "DATED: $rel" >> "$TMP.ev"; }
    continue; fi
  if [ "$f" = gone ] && [ "$fp" = "$k" ]; then                  # back, unchanged: nothing new to ingest
    printf -- '- %s · %s · %s → %s%s\n' "$rel" "$fp" "$gd" "$kf" "$att" >> "$V/$R"; echo "BACK: $rel" >> "$TMP.ev"; continue; fi
  if { [ "$f" = - ] || [ "$f" = gone ]; } && [ -s "./$rel" ]; then   # a path holding exactly a missing file's content: moved
    mvf=$(awk -F'\t' -v f="$fp" '$1 == f { print $2; exit }' "$TMP.miss")
    if [ -n "$mvf" ]; then
      st=$(last "$mvf"); st=${st#* }; st=${st#* }; st=${st#* }; mc=${st%% *}; ma=$(printf '%s' "$st" | sed -n 's/.*\( (attachment of [^)]*)\).*/\1/p')
      printf -- '- %s · %s · %s → %s%s (moved)\n' "$rel" "$fp" "$gd" "$mc" "$ma" >> "$V/$R"; echo "MOVED: $mvf → $rel" >> "$TMP.ev"
      awk -F'\t' -v p="$mvf" '$2 != p' "$TMP.miss" > "$TMP.m2" && mv "$TMP.m2" "$TMP.miss"; continue; fi
  fi
  if [ "$f" != - ] && [ "$f" != gone ] && LC_ALL=C grep -qI . "./$rel" 2>/dev/null; then   # changed only in whitespace: no new copy
    old=$(find "$V/raw" -maxdepth 2 -name "$kf" -type f 2>/dev/null | head -n 1)
    if [ -n "$old" ] && [ "$(nws "./$rel")" = "$(nws "$old")" ]; then
      printf -- '- %s · %s · %s → %s%s (same text)\n' "$rel" "$fp" "$gd" "$kf" "$att" >> "$V/$R"; echo "SAME TEXT: $rel" >> "$TMP.ev"; continue; fi
  fi
  file=${rel##*/}
  ...
  printf -- '- %s · %s · %s → %s%s\n' "$rel" "$fp" "$gd" "$stem.$ext" "$att" >> "$V/$R"   # a changed attachment stays one
  if [ "$f" = - ]; then echo "NEW: $rel" >> "$TMP.ev"; else echo "CHANGED: $rel" >> "$TMP.ev"; fi
done
[ -e "$TMP.more" ] || paths | while IFS= read -r rel; do
    [ -e "./$rel" ] && continue; st=$(last "$rel"); [ "${st%% *}" = gone ] && continue
    printf -- '- %s · gone · %s → -\n' "$rel" "$D" >> "$V/$R"; echo "GONE: $rel" >> "$TMP.ev"
  done
C=; for t in NEW CHANGED MOVED 'SAME TEXT' BACK GONE DATED RECENT; do C="$C$(grep -c "^$t:" "$TMP.ev") $(echo "$t" | tr 'A-Z' 'a-z') · "; done
[ -e "$TMP.more" ] || printf 'synced: %s · %s\n' "$D" "${C% · }" >> "$V/$R"
echo "${C% · } — record $R"
nn=$(grep -c '^NEW:' "$TMP.ev"); ng=$(( $(grep -c '^GONE:' "$TMP.ev") - $(grep -c '^MOVED:' "$TMP.ev") ))
[ -z "$FRESH" ] && [ "$nn" -ge 20 ] && [ "$ng" -ge 20 ] && echo "RESTRUCTURE?: $ng gone without moving and $nn new in one sync — ingest matches them before any new source page is written"
if [ -n "$FRESH" ]; then grep -v '^NEW:' "$TMP.ev"; else cat "$TMP.ev"; fi
rm -f "$TMP.ev" "$TMP.gd" "$TMP.more" "$TMP.miss" "$TMP.m2"
```

It never imports the vault into itself — a vault inside the folder, or the folder itself, is left out — and it checks both folders before writing anything. On a sync it leaves out files changed in the last two minutes (`RECENT`) — they may still be being written — and the next sync takes them. **A shell call is cut off after 120 seconds unless given more (180 at most):** the command stops itself after 100 seconds and prints `MORE` — run it again until it doesn't. It continues the same record and never copies a file twice.
...
- **Its duplicates.** A source page with the same `origin:` is the same source — or, for a moved file, the page whose `raw:` or `raw_previous:` names its copy (re-captures.md, *Finding the page*). A copy several paths' lines name — moved while still pending — takes its `origin:` from the newest path that isn't `gone`.
- **A copy whose path went `gone` before it was ingested** — and didn't move — is held back and named in the report or digest: the document left the folder before the wiki read it, so the owner decides whether it is still wanted. A moved-and-changed one is ingested as its successor's earlier copy, unread, onto `raw_previous:` (re-captures.md, *Moved and changed*). Compare the item's fingerprint with the line naming the copy behind the page's current `raw:` — for a binary, its original on `asset:`: the same is a duplicate; different is a re-capture that updates that page — `raw:` to the new copy, the old one onto `raw_previous:` (last ten; every capture gets its `## Version history` line), `published:` to the new date. When several copies of one origin are pending, ingest only the newest; the older ones go onto `raw_previous:` unread and leave the inbox with it.
...
- **Changed** files are copied again and get a new line; ingest updates their source page (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-ingest-pending/references/re-captures.md`).
- **Same text**: only whitespace changed — a line pointing at the existing copy, no new copy, nothing to ingest.
- **Moved**: exactly the content of a file no longer at its old path — a line pointing at that file's copy, marked `(moved)`, and the old path's `gone` line. Nothing to ingest; lint's mechanical fixes point the source page's `origin:` at the new path. A file moved *and* changed arrives as new, and ingest matches it to the page it came from (re-captures.md, *Moved and changed*). The record lines never hold an old path — paths can contain anything — so which path a copy moved from is read from the copy it names.
...
- **Dated**: an unchanged file that now has a git date — committed since the last sync — gets a line with the date; lint's mechanical fixes fill in its source page's `published:` if it had none.
- **Gone** from the folder: a `gone` line, reported once. Nothing in the wiki changes: a source whose document has gone — and hasn't moved — is **withdrawn**, which is read from the record, never written on the page. Lint lists the claims that rest on it alone, and the owner decides.
- **Recent**: skipped, for the next sync.

**`RESTRUCTURE?`** — twenty or more gone and twenty or more new in one sync — usually means the folder was reorganised. Ingest matches the new files to the gone ones before it writes a single new source page (re-captures.md, *A reorganised folder*).
