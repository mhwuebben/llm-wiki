...
**0. Scope.** A clip, a sync or a drag-and-drop never went through capture's scope check, so ingest applies it: the schema's §1 out-of-scope list — admin, and another living person's personal data. A file that fails stays in `raw/inbox/`, untouched, until the person decides; one whose provenance already carries `scope: "override — …"` was decided at capture.
