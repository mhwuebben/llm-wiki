...
- If the vault folder can't be reached — no linked computer, the folder not connected — don't drop a link or a note I send: put it on the backlog in the project doc wiki-backlog.md, one line each — the date · the part it is for, where more than one is in play · ingest, or save if I said only to save it · the link, not fetched, or "pasted note" with my text verbatim indented below it, or a file's name with re-attach — and tell me. Whenever the folder can be reached and wiki-backlog.md has lines, drain it before anything else I ask, as the wiki-capture-only skill says: each line by its route, oldest first, and ask me for any file marked re-attach.

- Before capturing anything, check it against the out-of-scope list in schema §1 — my list, and the whole gate: what it doesn't name is filed. If an item matches it, ask me once, file it anyway or skip it, and do what I say. If I say to file it anyway — or say so up front: "capture this anyway", "force it" — capture and ingest it like any other item, with scope: "override — <the line it matched>" in its provenance so the decision is on record.
...
The exception, not the default: one project that deliberately combines several wikis, each a **part** of one brain (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-query/references/parts.md`). Use the text above unchanged, with its first line naming the **home** part — the one a capture goes to by default — and insert this paragraph directly after that first paragraph:

```
That wiki is the home part of a brain made of several wikis, all connected to this project: {{PARTS — one entry each: name (vault id) — what its schema §1 says it is for}}. If one of them isn't connected, say so whenever it would have been consulted, rather than answering from the others as if they were all there. Read across the parts a question needs — routed by what each schema's §1 says that wiki is for, and scoped exactly when I name one or a few of them. This project writes nothing into any wiki but captures: a link, a file or a note lands in one part's raw/inbox — the home part unless I say otherwise or it plainly belongs to another — and is ingested from that part's own project, by its scheduled run or when I ask there; filing an answer is a capture too. Ingest, lint, maintain, dream passes, folder imports and schema upgrades don't run here: say which part's project to use instead. A claim taken from another part is quoted with that part's name and vault id, never linked, because links don't resolve across folders; a contradiction between two parts is reported to me, not written into either. Say which parts an answer used and which it left out. Everything below is about one part at a time unless it says otherwise, and where it says to capture and ingest, here it means capture: answer from the captured text, and say it is still pending.
```

The list is what makes a missing part visible: without it, a folder nobody connected simply isn't in the brain, and an answer from two of three parts reads exactly like one from all three. Each part keeps its own project, with the one-wiki text above, and its own scheduled tasks, each with only its own folder attached and its own vault id in the prompt. A combined project gets no scheduled task: one with several wikis attached would stop before its first write.
...
- **Questions about the wiki itself route to their own skills.** "How's my wiki doing?" and "any duplicates?" are questions too; without this rule they go to wiki-query, which answers from the pages rather than checking the vault. Questions about the plugin — "how do I turn on debug mode?", "why is there a ring around my graph?" — go to wiki-help, which answers from the plugin itself instead of improvising, and never proposes changing the wiki to make a view look different.
...
- **Several wikis in one project** (only when that paragraph is added). People keep separate vaults for good reasons — different scope rules, different readers — and the separation is worth keeping. Reading across them is what makes the split cheap; writing across them is what would make it dangerous, since a link only resolves inside one folder and each vault's scope rules are its own. So a combined project is a reading room: the only write it makes is a capture into one part's inbox, which that part's own ingest then checks against its own scope. Everything that works on a vault as a whole runs where that vault is the only one connected, and so never has to guess which one it is in.
- **The scope test.** Without it the routing rule above is unconditional: every file becomes a source. Ingest is not parking — it writes a source page, propagates across the index and overview, and leaves the content in a dozen greppable files that lint reads and any deck or export draws on. What should never get there is the owner's call, made at setup and changeable any time; the rule makes a session look at their list, ask once when something matches, and then do what they say — never refuse on its own judgement, and never file on it either.
