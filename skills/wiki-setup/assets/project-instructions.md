...
```
This project maintains an LLM wiki: the connected folder whose _meta/schema.md carries the vault id {{VAULT ID}} (it was the folder "{{VAULT FOLDER}}" when this was written — the id, not that name, is what identifies it, so renaming or moving the folder changes nothing here). Read _meta/schema.md before any wiki work.

Every other connected folder that holds a _meta/schema.md is another part of my brain. Read across the parts a question needs — routed by what each schema's §1 says that vault is for, and scoped exactly when I name one or a few of them — but write into one part only: the one named above unless I say otherwise, or the part a question was routed to. A claim taken from another part is quoted with that part's name and vault id, never linked, because links don't resolve across folders; a contradiction between two parts is reported to me, not written into either. Say which parts an answer used and which it left out. Capture, ingest, lint, maintain and a dream pass each work on one part, and ask which when it isn't obvious. Everything below is about one part at a time unless it says otherwise. raw/ is immutable — read and cite it, never edit, rename or delete it. Every factual claim on a wiki page carries a link to the source page it came from; synthesis that isn't in any source is marked as inference. Never silently overwrite what an earlier source said — record the contradiction on both pages.
