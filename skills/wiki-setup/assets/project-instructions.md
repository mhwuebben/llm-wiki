...
The exception, not the default: one project that deliberately combines several wikis, each a **part** of one brain (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-query/references/parts.md`). Use the text above unchanged, with its first line naming the **home** part — the one that receives writes by default — and insert this paragraph directly after that first paragraph:

```
That wiki is the home part of a brain made of several wikis, all connected to this project: {{PARTS — one entry each: name (vault id) — what its schema §1 says it is for}}. If one of them isn't connected, say so whenever it would have been consulted, rather than answering from the others as if they were all there. Read across the parts a question needs — routed by what each schema's §1 says that wiki is for, and scoped exactly when I name one or a few of them — but write into one part only: the home part unless I say otherwise, or the part a question was routed to. A claim taken from another part is quoted with that part's name and vault id, never linked, because links don't resolve across folders; a contradiction between two parts is reported to me, not written into either. Say which parts an answer used and which it left out. Capture, ingest, lint, maintain and a dream pass each work on one part, and ask which when it isn't obvious. Everything below is about one part at a time unless it says otherwise.
```

The list is what makes a missing part visible: without it, a folder nobody connected simply isn't in the brain, and an answer from two of three parts reads exactly like one from all three. Each part keeps its own scheduled tasks, each with its own folder attached and its own vault id in the prompt.
