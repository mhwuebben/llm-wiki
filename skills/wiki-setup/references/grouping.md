...
1. **§3c's line**, when it plainly settles it from the source itself — the book, module, project or engagement the source says it is part of. Where each page has one subject by its nature (a chapter's book, a lecture's module), this is the rule that decides. When the line settles it, the vote isn't run, and lint never proposes against it.
...
Used only with the person present: adopting a grouping, a subject renamed, merged or split, going back to flat, a type split, or a move they asked for. Take the vault lock (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-setup/references/locking.md`) after the approval and the pre-flight, hold it through the schema and frontmatter changes, the moves, the path updates and the index, renewing it every few dozen files, and release it at the end — unless the skill running this, an upgrade, already holds it.
...
4. **Record each move as it happens:** right after a move is verified, append `old path → new path` to `_meta/moves/YYYY-MM-DD.md`. The record is what an undo replays, what the check in step 7 reads, and what an interrupted run resumes from.
