...
Operations logged: `setup`, `capture`, `ingest`, `query`, `lint`, `maintain`, `dream`, `schema`. A `query` entry is written when an answer is filed as a note, or when the core of a question went unanswered — then with the topic, not the question word for word, and a `- gap:` line.
