...
Ask all four in one message, then stop and wait — 2 and 4 with AskUserQuestion, 1 and 3 as free text, since their answers are names and questions only the person can supply:

1. **What is this wiki about, and whose is it?** (a research topic, a book, a company/market, themselves, a course, a hobby — and the owner's name, or the team's; schema §1 records it)
2. **What goes in?** (papers/PDFs, web articles, podcast or meeting transcripts, own notes, spreadsheets, images)
3. **What will you ask it later?** — one or two real questions. These shape the page types more than anything else. Let them type their own; whatever they write goes into the schema word for word, so canned options would put words in their mouth.
4. **How involved do you want to be?** (review each source before it's filed / file first and report after)

Pick the closest domain preset in `references/domain-presets.md` and apply all of it in Steps 3–5 — its page types and vocabulary, its starting grouping, and any extra root page, template or frontmatter field it adds. Don't ask more than four questions; anything else you can infer or settle later — including the languages the sources will be in, for schema §1, from their answers and the first documents.
...
- **Has documents already** → build the structure, then offer to copy (never move, never rename originals destructively) the existing documents into `raw/inbox/` for later ingest. Say how many files you found and what types.
...
Each type folder starts flat unless the preset groups it (schema §3, **Grouped by**); a grouped folder gets its subfolders as pages arrive, not in advance. Copy the four page templates from `assets/templates/` into `_meta/templates/`, copy `assets/wiki-lock.sh` to `_meta/wiki-lock.sh`, and run `sh _meta/wiki-lock.sh status` once, which creates `_meta/wiki-lock.md` in its free form (`references/locking.md`). Every skill that changes the wiki takes that lock first. Create every folder with at least one file in it (a `.gitkeep`, the seed page, or — in `raw/`, its subfolders and `outputs/` only — a short `README.md` saying what the folder is for) so the structure survives a sync and is visible in Obsidian. Lint counts none of these as a source. Under `wiki/`, a README would be linted as a page, so use `.gitkeep` there.
...
- the `Languages:` line in §1; §11's Ingest line from their answer to question 4; and §11's Query line on whether unanswered questions are logged (the personal preset says not)
...
3. Tell them exactly where each goes. The instructions: the project instructions field of the Claude project that owns this vault (Claude desktop or claude.ai → the project → Settings), or a `CLAUDE.md` at the vault root in Claude Code. The prompt: a new scheduled task on that project, set to approve automatically — otherwise a scheduled run stops at its first file write — and to run on the computer that holds the folder.
...
## Step 9 — Settle the routines

1. **The cadence.** Weekly suits an active vault, monthly a quiet one; ask what fits theirs rather than assuming, and update the Maintain line in schema §11 to match the task they create.
2. **If their Step 1 answer was to review each source before it is filed**, say plainly what that means for the routine: a scheduled wiki-maintain run will only list what is pending, and ingesting waits until they run wiki-ingest-pending themselves. Offer to switch the Ingest line in §11 to file-first if that isn't what they want.
3. **Later, once there are ten or so sources:** a monthly dream pass that proposes connections for them to decide on. Mention it now, with the wiki-dream-only prompt from *The scheduled task*; don't schedule it on an empty vault. Its reports wait for them, and wiki-dream-ingest works through each one with them.

Close by telling them their job — curate sources, ask good questions, decide what matters — and yours: the reading, summarising, cross-referencing, filing and bookkeeping. Then, once any question from Step 9 is settled, show the **Closing** block of `assets/about.md`, word for word and rendered, as the last thing setup says.
...
2. List what the template has that their schema lacks or states differently — a missing field, rule, log operation or §11 line — each with the section and the one line of text you'd add or change. Skip anything they deliberately customised: their own page types, their own questions, their own wording of a rule that means the same. For an added line that needs a value — a `Languages:` line, say — propose one inferred from the vault. If a type folder is past the threshold in §10 (100 pages if it sets none), offer a grouping, and adopt it only by `references/grouping.md` (*Adopting a grouping*), with the person present. Where §3c lists subjects without a hub, offer one for each subject whose value is also a page's name or alias — it only adds the link to §3c; no page changes and nothing moves.
