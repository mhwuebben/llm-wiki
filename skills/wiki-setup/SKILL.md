...
- The vault is the connected folder. If no folder is connected, ask the person to click **Work in a project or folder** and pick (or create) one — everything below happens inside it. **One wiki per project is the default.** If a connected folder already holds a `_meta/schema.md`, this project already has a wiki: say so, and **stop** — the new one is set up from a project of its own, where it is the only wiki, since a session that can reach another folder holding a `_meta/schema.md` writes nothing but captures (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-query/references/parts.md`, *A project that combines several wikis*). If the person means to combine them, say what happens then: set the new one up in its own project, with a §1 that says plainly what belongs in it — that sentence is what later routes questions between parts — then connect it to the combined project and ask there for the combined instructions (*Several wikis in one project*, below).
...
3. **Second card, built from the answers so far.**
   ...
   - **What should never be filed here?** Several can be picked; nothing is pre-chosen, and the free-text field takes their own ("anything under NDA", "client names"). **Credentials and keys** — passwords, API keys, account numbers *(Recommended: once filed they are copied onto pages and into exports)*; **Admin paperwork** — tickets, bookings, invoices, receipts, statements; **Other people's personal data** — CVs, IDs, medical or financial records, private messages, contact files, photos of others; **Nothing — file whatever I hand over**. What they choose becomes schema §1's out-of-scope list, in their words, and is the whole gate. Say in the question that any single item can still be filed anyway, and that the list can be changed later.
...
│   ├── wiki-search.sh     the searches the skills run — names, backlinks, citations, pending, raw
│   ├── webclipper-template.json  the Web Clipper template, written when Step 6 sets it up
...
- the one-paragraph statement of what this wiki is for — what it is *not* for is §1's out-of-scope list, from Step 1, and nothing else
...
- §1's **Out of scope** list: exactly what they chose in Step 1, one line each, in their words — nothing added. "Nothing" writes `nothing: file whatever I hand over`
- §1's **Vault id** — the one minted in Step 3, which is what the project instructions and every scheduled task will point at — and §12's **Built with:** line, this plugin's version from `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json` and today's date. Neither is ever edited afterwards except by an upgrade, which updates the version line only; wiki-doctor reads both.
...
1. List the parts — every connected folder holding a `_meta/schema.md`: its folder name, its vault id, and what its §1 says it is for. A part with no vault id is named as needing one, minted by upgrade mode from that part's own project; a §1 too vague to route by is worth a sentence to the person, since routing reads nothing else.
2. Ask which part is **home** — the one a capture goes to when nobody says otherwise. Recommend the one the current project instructions already name, if any.
3. Hand over `assets/project-instructions.md` with the home part's id and folder in the first line and the *Several wikis in one project* paragraph inserted after the first paragraph, its list filled in. Say that each part keeps its own project and its own scheduled tasks, each with its own folder attached — and that a combined project gets no scheduled task of its own.

This writes nothing into any vault: it produces text to paste and changes no schema. It is also the only thing wiki-setup does in a combined project, and worth saying when handing the text over: here, questions read across every part and a capture lands in one part's inbox; ingest, lint, maintain, dream passes and upgrades run from each part's own project, and each part's scheduled tasks stay there.

## Changing what is blocked

Schema §1's **Out of scope** list is the owner's, and they change it by asking: *"stop blocking admin paperwork"*, *"also block anything under NDA"*. Show the list as it is and as it will be, take the vault lock, edit §1 in their words, append a line to §12, and log a `schema` entry naming the change. It writes the schema, so like an upgrade it runs only where this is the only wiki connected: in a project that combines several, name the part's own project. Nothing already filed moves: an addition is checked against existing sources by the next lint pass (check 13), which proposes and never removes on its own; a removal simply stops the question being asked.
...
**Only where it is the only wiki connected.** An upgrade writes into a vault's schema, scripts and pages, so a session that reaches two or more folders holding a `_meta/schema.md` doesn't run one: name the parts, and say that each is upgraded from its own project — wiki-doctor, which does run here, says which of them are behind.

1. Read their `_meta/schema.md` and `references/schema-template.md` side by side, section by section, plus `_meta/templates/` against `assets/templates/` and their `README.md` against `assets/vault/README.md`. Where `${CLAUDE_PLUGIN_ROOT}/CHANGELOG.md` has an *Upgrading an existing vault* note for a release after the version §12's **Built with:** line names — or, with no such line, for a release dated on or after its `setup` log entry — read that too. Not the last `schema` entry: changing what is blocked or adopting a grouping logs one too, and would hide the notes of every release before it.
2. List what the template has that their schema lacks or states differently — a missing field, rule, log operation or §11 line — each with the section and the one line of text you'd add or change. Skip anything they deliberately customised: their own page types, their own questions, their own wording of a rule that means the same. For an added line that needs a value — a `Languages:` line, say — propose one inferred from the vault. §11's Ingest, Maintain and Dream lines hold the owner's settings — the filing mode, the cadence: keep those values, and propose only wording the template added around them. **§1's Out of scope** is the owner's list: where theirs still carries a fixed "always" list from an earlier template, ask the Step 1 question — *What should never be filed here?* — with their current categories named, and write what they choose, in their words. Replace the whole old paragraph with their list and the template's paragraph after it: its fixed categories and override sentences came from the template, not from them, and left beside the new paragraph they would contradict it. If a type folder is past the threshold in §10 (100 pages if it sets none), offer a grouping, and adopt it only by `references/grouping.md` (*Adopting a grouping*), with the person present. Where §3c lists subjects without a hub, offer one for each subject whose value is also a page's name or alias — it only adds the link to §3c; no page changes and nothing moves.
