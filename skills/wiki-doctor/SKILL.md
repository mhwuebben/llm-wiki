...
**2. Identity and parts.** Where several connected folders hold a `_meta/schema.md`, they are parts of one brain (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-query/references/parts.md`): list them — folder, id, what §1 says each is for — and check this one against them. A part whose §1 is vague or missing cannot be routed to, which is worth saying: routing reads that section and nothing else. Then, for the vault under examination: schema §1 carries a **vault id** (`**Vault id:** wiki-…`), which is what the instructions and the task prompts point at, because folder names change. Missing → wiki-setup's upgrade adds one. Present, but another connected folder's schema carries the same id → two copies of one vault: name both folders and ask which is live, because two sessions writing two copies is the one failure the vault lock cannot catch.
