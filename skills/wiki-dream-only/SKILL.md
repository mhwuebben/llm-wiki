...
5. **Pattern** — a loop that now shows up in a third source, which is the bar `patterns.md` sets for a place on the page. Includes **drift**: a claim that has moved twice or more across a source's `## Version history` lines and the `## History` of the pages carrying it. Skip when the vault has no `patterns.md`.
