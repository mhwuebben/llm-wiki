...
- Length: most pages 300–800 words. A source page read in sections (`sections:` in its frontmatter) groups its key claims under the section labels and is never split. Any other page past schema §10's **Split past** line (~1,200 words if it has none), not counting `## History` or `## Version history`, split along the natural seam and link both halves — `references/propagation.md`, *Splitting and merging*.
- **A history companion** — `<page>-history`, beside its page and of the same type, with `history-of: "[[<page>]]"` in its frontmatter — holds the older `## History` entries of a page whose history outgrew it, oldest last, each entry moved whole with its date, reason and citations. Its frontmatter: `type:` and `title:` as its page's ("<title> — history"), `history-of:`, `created:`, `updated:` and `sources:` for what its entries cite — no `status:`. The page keeps its newest five entries and ends its `## History` with a line linking the companion. It is part of the page, not a second page on the subject: it gets an index row under its page, and is never merged, split or reported as thin or bloated.
