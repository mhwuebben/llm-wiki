...
Do it serially, and keep it short. One run takes about eight items at most — or one large source, on its own (`large-sources.md`); re-captures don't count, since each is compared rather than read (`re-captures.md`) — however many were approved — past that, quality slides as context fills. Offer to split the rest across sessions rather than degrading quietly; unattended, leave them pending and say so.
...
- **Cap it.** A batch that would create more than ~20 new pages — entity, concept and other named pages; source pages don't count — stops there (a wave already drafted is finished — step 2): ingest the items that fit, oldest first, and leave the rest pending. With the person present, offer the rest after the report, as a new pass — not as a question in the middle of one, while the lock is held. **Re-captures don't count against it** — they update pages that already exist, merged into one edit per page, however many there are (`re-captures.md`). **A large source is one item and is never cut in half** (`large-sources.md`, *In a batch, and unattended*): it takes its place in the order like any item, is checked against the cap when its turn comes, and once started is finished in the same run.
