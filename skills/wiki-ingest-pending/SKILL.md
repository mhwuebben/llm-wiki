...
4. Check it isn't already ingested: does a source page — any page with `type: source`, normally under `wiki/sources/` but wherever the owner put it — carry a `raw:` or `raw_previous:` entry pointing at this file, or cover the same URL, or the same `origin:` from an imported folder, or the same `export:` item key — or the same title *and* author *and* edition or period? Files clipped twice, or ingested without being moved out of the inbox, are common. A recurring title is not a duplicate: this year's annual report is a new source with its own name, not a re-capture of last year's. **Match by filename too:** a page whose `raw:` or `asset:` names this file — in `raw/` or `raw/assets/` — while the file still sits in `raw/inbox/` is an interrupted ingest: finish it (Step 4 for anything not yet propagated, then Step 5's move) rather than treating it as done — unless another session holds a live lease on an ingest or a maintain run (a lease this run holds doesn't count): it may still be ingesting it, so leave it alone. If it is already in and unchanged, say so: there is nothing to ingest. A **changed version** of a source already in — same URL, or same title, author and edition or period, but different content: a re-capture — is not a duplicate. It updates that source page: point `raw:` at the newest capture, move the old path onto `raw_previous:` (newest first), add the new capture's files to `asset:` while keeping the earlier ones, say on the page what changed, and propagate only the differences. **Get the differences by diffing, not by re-reading:** the old capture is still in `raw/` under the path now on `raw_previous:`, so `diff -u raw/<old> raw/inbox/<new>` (or the file tools, comparing section by section) says exactly what moved — the rest of the page, and every page citing it, is already correct. Read the whole new capture only when the diff is most of the file. A source that is re-captured often — a docs folder synced every week — is the normal case for this path, not the exception. `references/re-captures.md` has the whole of it: finding the page (a moved file included), the change extract, one edit per page however many re-captures a run holds, contradictions the new version settles, and **echoes** — a new version repeating what the wiki itself concluded, which is recorded and never counted as independent confirmation.

   **Record the drift on the page.** Add a line at the top of its `## Version history` section (creating the section if the page predates it: one line per path already on `raw_previous:`, oldest at the bottom as `— captured before this section existed`, under a `- <created> · <the first capture> — first capture` line, and only then trim the list — a capture named in neither place reads as an orphan to lint check 11 forever): the capture's date, its file in `raw/`, what changed in the claims in one sentence — `no claim changed` for formatting or typo churn — and the reason the source itself states, or `reason not stated`. **Never infer the reason**; a plausible invented one is worse than the admission, and an unexplained reversal that matters is a gap worth naming in the report. Then keep `raw_previous:` to the last ten paths, newest first: the older captures stay in `raw/` and stay listed in the section, which is what keeps every file accounted for (lint check 11). One line per capture, so the section is the ledger and the frontmatter is only the recent working set. A redaction line on the page stays, and what it removed is not brought back from the new capture.
5. Check it against the schema's out-of-scope list — the same test wiki-capture-only applies (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-capture-only/SKILL.md`, step 2), since a file that arrived by clipper, sync or drag-and-drop never met it. It matches a line → ask once, *file it anyway* or *skip it*, in the list of items when there are several; the other items go ahead meanwhile. Ingested anyway — or named up front, *"ingest it anyway"* — it is an ordinary source, propagated as usual, with `scope: "override — <the line it matched>"` in its frontmatter. A provenance block that already carries that line was decided at capture: carry it onto the source page and go on. Unattended, nobody can answer: leave the item pending, untouched, and name it with its line in the report. A run never overrides on its own.
6. **The vault lock** (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-setup/references/locking.md`) is taken after Step 2's check-in — for several items, after the list's yes, before the readers are dispatched, or, where §11 asks to discuss each source first, after the last of those check-ins (readers may draft source pages before it: they are new files) — and never before: Steps 1 and 2 only read, and no one should wait on a conversation. Inside an unattended wiki-maintain run, use the token it holds. Once you have it, re-read `index.md` and re-run item 4 for each item: another session may have written in the meantime. Renew it at each item and each step; before a long read, or before waiting on sub-agents, renew it for as long as that takes. Release it once Step 5b is done, before the report — unless wiki-maintain holds it.
...
- **An export** — one file holding many independent items (Kindle clippings, a read-later export, an `.mbox`, a chat export): split it first, by `${CLAUDE_PLUGIN_ROOT}/skills/wiki-capture-only/references/source-types.md`, *Exports* — splitting is capture, and needs no lock — then ingest its items as pending items, and give the export its own short page in the same pass. An export whose items already carry an `export:` line naming it has been split: write its page and move it, nothing more.
- **A password, API key or account number inside**: wiki-capture-only's fidelity rule applies (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-capture-only/SKILL.md`, *Secrets inside a document*) — kept off the source page where §1's list names credentials; otherwise one line of warning, in the report or digest when unattended, and the source is filed as it is.
...
2. **Create pages** for entities and concepts that now clear the promotion bar in the schema. Say each page as it is created — `+ wiki/concepts/self-attention.md` (`references/batch-ingest.md`, *Showing progress*); the same goes for the source page in Step 3 and the moves in Step 5. New pages start small and honest: `status: stub` is fine. Place each by schema §3, as the source page was. Only a type §3 groups by subject carries `subject:` (grouping.md, *Assigning a subject*); a new page never inherits the source's.
...
6. **Close the list** (`references/propagation.md`, *Close the list*). Every named page that got a claim is on the source page's `## Entities and concepts`, and every page on it cites the source in its body — tested with the same command lint check 6 uses. Fix any gap now. A page with no `## Entities and concepts` section at all fails this step rather than passing it with nothing to test — write the section, then check. Don't start Step 5 until the list is closed. Ingesting several items, run that check once for the whole pass rather than once per source (`references/batch-ingest.md`, step 4) — and still let nothing out of the inbox until it has passed.

**Notes are the exception.** A note (`type: note`) is a dated answer: ingest doesn't rewrite it. If the new source contradicts a claim a note makes, the note gets the §7 contradiction callout and nothing else — not even a new `answered:` date. A re-capture adds the two things `references/re-captures.md` names: the *Adopted in* line on a note whose inference the new version adopts, and a settled callout moved to `## History`. `references/propagation.md` says how to find those notes.
...
- `index.md`: rows for new pages, updated summaries and dates for changed ones — for several items, rebuilt once at the end of the pass, not after each (`references/batch-ingest.md`, step 5).
...
## Step 5b — Verify what this pass promised

Three things ingest promises and otherwise only assumes. Run them over the pages this pass wrote — not the vault — once bookkeeping is done, so the index and the log are there to check against. They are lint's checks on a delta, and they cost a few greps over pages you have just written. A mistake caught here costs a minute; the same mistake found next month costs an afternoon.

1. **No orphan was created** (check 2): every new page has at least one inbound link from another page's **body**. `sh _meta/wiki-search.sh backlinks <new page>`, then, on what it returns minus `index.md`, `overview.md` and the other frame pages, `sh _meta/wiki-search.sh cites <new page> <those files>` — an index row and a `sources:` line are not inbound links, which is exactly what check 2 says. Nothing left means the link this skill promised (*Links*, in `references/propagation.md`) was never added.
2. **The index matches** (check 8): a row for every page created, and the summary and date right for every page changed.
3. **The frontmatter is complete** (check 9's mechanical fields) on the pages this pass wrote: `type`, `title`, `created`, `updated`, `status`, plus `sources:` on an entity, concept or note page and `raw:`/`asset:` on a source page.

Fix what they find, here — all three are mechanical. Then add one line to the log entry: `- verified: 6 new pages · 0 orphans · index in line · 1 frontmatter field fixed`, so the log says the pass checked itself. Anything bigger than a mechanical fix — a page that should be merged or split — is named in the report and left for a lint pass, which runs its checks over everything regardless of what happened here; this step repairs what this pass broke, nothing else.

**What this step is not.** It does not replace check 3: a page created under a synonym of an existing one is caught **before creation**, by the same-name and alias search in *Quality rules*, and what that misses — a near-synonym, two pages with the same summary — needs the reading a lint pass does. Nor does it replace check 9's judgement half, or any check over the rest of the vault.
...
- **Watch for the same thing under two names** — a new page that duplicates an existing one under a synonym is the most common way these wikis rot. Search the vault by name and alias before creating — plus the pages this run has already created, which the index doesn't know about yet (`references/propagation.md`, *Build the touch list first*).
...
- `references/re-captures.md` — a newer copy of a source already in: finding its page, comparing instead of re-reading, one edit per page, settled contradictions, echoes of the wiki's own conclusions, and a reorganised folder. Read when an item is a re-capture.
