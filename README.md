...
- **Folders you already have.** An export that packs many items into one file — Kindle highlights, a mailbox — becomes one source per item. A docs repository or an exported folder is imported whole, with a record of where each file came from, and every scheduled run brings in what changed there — in one run even when the whole folder was regenerated. Moved files keep their pages, files that disappear are reported rather than deleted, and when a new version repeats something the wiki itself concluded, the wiki records that instead of counting it as fresh evidence.
