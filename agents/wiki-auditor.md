...
- Checks 1, 2, 3, 4 and 8 — and check 6's propagation gaps in both directions, which group by page or by source, check 9's subject vote and check 10's subject proposals, which read other source pages' subjects — need the whole vault's page names and inbound links to answer. If your slice is a folder, collect link targets and filenames across the vault first, then evaluate only your slice's pages. If you cannot, say which checks you could not complete instead of reporting them clean.
