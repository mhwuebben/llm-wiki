...
Change authorship where it appears: `author` and `repository` in `.claude-plugin/plugin.json`; `owner` and `name` in `.claude-plugin/marketplace.json` (and the install commands above, which use that name); `skills/wiki-setup/assets/about.md`, the introduction and sign-off `wiki-setup` shows; the closing line of `skills/wiki-status/SKILL.md` and `skills/wiki-doctor/SKILL.md`; and the byline and contact lines in `README.md`. Keep the copyright line in `LICENSE`, as the MIT licence requires, and add your own beside it.

### Upgrading

A sync updates the skills. It does not touch anything in your vault:

- **Your `_meta/schema.md` stays as it was.** It is yours, and the skills follow it over their own defaults — lint included, so a schema you have customised never floods a report. When a release changes a schema convention, ask Claude to *upgrade the vault*, in that wiki's own project — never one that combines several: `wiki-setup` compares your schema with the current template and proposes each change for you to approve, brings existing pages in line with what changed, replaces `_meta/wiki-search.sh`, `_meta/wiki-lock.sh` and `_meta/templates/` with the plugin's current copies (keeping anything you added to a template), and offers subfolders for any folder that has grown large. A schema that still carries the fixed out-of-scope list of an older release gets the setup question again — *what should never be filed here?* — and your answer replaces it.
- **Your project instructions stay as they were.** When a release changes routing, re-paste them: ask Claude to *upgrade the vault*, which shows the current text, or copy the first fenced block from `skills/wiki-setup/assets/project-instructions.md` and replace `{{VAULT ID}}` and `{{VAULT FOLDER}}` with your vault's id (schema §1) and folder name.
